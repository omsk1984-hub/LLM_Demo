...
- Перед реализацией любой задачи создавай план в `plans/*.md`
- План должен быть утверждён пользователем перед началом написания кода
- Если сомневаешься — задай уточняющий вопрос
- Всегда подключай зависимости через DI и конструктор, используй ServiceProvider только при необходимости
- Выполняй curl через curl.exe